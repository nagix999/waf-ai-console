...
export default function RetryAnalysis({ detail, onOpen }) {
  ...
  return <div className="retry-actions ux-toolbar">
    ...
    <Dialog open={open} title="실패한 분석 재실행" onClose={() => { if (!busy) setOpen(false); }}>
      ...
      {info && <><p>기존 실패 이력은 보존하고 새 실행을 연결합니다.{detail.analysis_purpose === "test" ? " 재실행 결과는 테스트 결과와 평가 지표에 반영됩니다. 저장한 평가 기록은 유지됩니다." : " 기존 운영 분석은 그대로 유지합니다."}</p><dl className="label-metadata"><dt>모델</dt><dd>{info.model_name || info.model_profile || "복원할 수 없음"}</dd><dt>지침</dt><dd>{info.prompt_version || "복원할 수 없음"}</dd></dl>
        ...
        {info.allowed && <><p className="notice">모델을 다시 호출하므로 처리 자원과 비용이 발생할 수 있습니다.{[info.provider, info.verifier_provider, info.evidence_editor_provider].includes("openai") && " 원문 또는 원문을 포함한 분석 내용이 마스킹 없이 OpenAI로 다시 전송될 수 있습니다."}</p><label className="checkbox-row"><input type="checkbox" checked={approved} disabled={busy} onChange={event => setApproved(event.target.checked)} />당시 모델·지침과 호출 비용을 확인했습니다.</label><button type="button" className="primary" disabled={!approved || busy} onClick={submit}>{busy ? "접수 중…" : "재실행 시작"}</button></>}
      </>}
