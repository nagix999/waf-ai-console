...
function useRead(read, dependencies = []) {
...
const actions = {
  capture_production_baseline: ["이전 운영 구성 기록", "Previous Production baseline captured"], promote_production: ["운영 승격", "Promoted to Production"],
...
export function Overview({ onNavigate, onOpen }) {
  ...
  return <div className="v5-overview page-stack"><div className="ux-toolbar"><label>API Key<select value={key} onChange={e => setKey(e.target.value)}><option value="">{w("전체 운영 키", "All Production keys")}</option>{connections.value?.productionKeys?.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><span className="v5-context">{w("요청·실패·대기만 키별로 조회합니다. 공식 평가는 운영 구성 기준입니다.", "Key filter applies to requests, failures and queue only. Official evaluation is configuration-wide.")}</span></div><ReadState error={runtime.error || production.error} />
    ...
    <div className="v5-overview-columns"><div className="page-stack"><Section title={w("공식 운영 평가", "Official Production evaluation")} action={<button className="text-button" onClick={() => onNavigate("quality")}>{w("평가 보기", "View evaluation")} →</button>}><OfficialMetrics evaluation={p?.evaluation} /></Section>
...
export function Promotion({ initialRunId, onNavigate, onBack }) {
  ...
  return <div className="page-stack v5-promotion"><button className="back" disabled={busy} onClick={onBack}>← {w("테스트로 돌아가기", "Back to test")}</button><Section title={data?.candidate_name || w("승격 조건 확인", "Checking candidate")} context={w("이 테스트에서 실행한 구성과 공식 평가를 검토합니다. 다른 테스트를 임의로 선택하지 않습니다.", "Review this test's captured configuration and official evaluation.")} action={<button className="text-button" disabled={busy} onClick={() => setRevision(n => n + 1)}>{w("다시 확인", "Refresh checks")}</button>}><p className="v5-context">{w("모델·분석 지침·입력 스키마를 하나의 구성으로 적용합니다.", "Models, analysis instructions and input schema are promoted as one configuration.")}</p></Section>
    ...
    {data && <><Section title={w("운영 구성 비교", "Configuration comparison")} action={<button className="text-button" onClick={() => setTechnical(true)}>{w("버전·식별정보", "Versions & identifiers")}</button>}><div className="v5-comparison"><div><h3>Current Production</h3><ConfigurationRows snapshot={data.current.snapshot} names={data.current.profile_names} versions={data.current.version_names} /></div><div><h3>Candidate · {data.candidate_name}</h3><ConfigurationRows snapshot={data.candidate} names={data.current.profile_names} versions={data.current.version_names} /></div></div><div className="v5-diff-strip">{["primary", "verifier", "evidence_editor", "prompt", "input_schema"].map(key => <span key={key} className={JSON.stringify(data.current.snapshot?.[key]) === JSON.stringify(data.candidate?.[key]) ? "" : "v5-changed"}>{key} · {JSON.stringify(data.current.snapshot?.[key]) === JSON.stringify(data.candidate?.[key]) ? w("동일", "Unchanged") : w("변경", "Changed")}</span>)}</div></Section>
      ...
      <Section title={w("승격 전 확인", "Preflight")}><ul className="v5-preflight">{data.checks.map(check => <li key={check.code}><Icon name={check.passed ? "check" : "alert"} size={18} /><span>{checkNames[check.code] ? w(...checkNames[check.code]) : check.code}</span><strong>{check.passed ? w("확인", "Pass") : w("미충족", "Blocked")}</strong></li>)}</ul>
...
export function ProductionEvaluation({ onOpenRun, referenceProps }) {
  ...
  return <div className="page-stack"><Section title={w("공식 운영 평가", "Official Production evaluation")} action={<button className="text-button" onClick={refresh}>{w("새로고침", "Refresh")}</button>}><ReadState error={error} /><OfficialMetrics evaluation={evaluation} /></Section>
...
export function RuntimeWorkspace({ onOpen }) {
  ...
  return <div className="page-stack"><div className="ux-toolbar"><label>{w("기간", "Window")}<select value={window} onChange={e => setWindow(e.target.value)}>{["1h", "6h", "24h", "7d"].map(v => <option key={v}>{v}</option>)}</select></label><label>{w("용도", "Purpose")}<select value={purpose} onChange={e => setPurpose(e.target.value)}><option value="all">{w("전체", "All")}</option><option value="production">Production</option><option value="test">Test</option></select></label><small>{w("마지막 갱신", "Updated")}: {formatDate(r?.updated_at)}</small></div><ReadState error={error} />
    ...
    <Section title={w("현재 운영 구성", "Current Production configuration")} action={<button className="text-button" disabled={!config} onClick={() => setConfigurationOpen(true)}>{w("구성 상세", "Configuration details")} →</button>}><p className="v5-context">Primary · {config?.profile_names?.[config?.snapshot?.primary?.profile_id] || "—"} · {config?.configuration_id ? `${w("승격", "Promoted")} ${formatDate(config.applied_at)}` : w("기존 운영 구성 · 첫 승격 전", "Previous Production configuration · not yet promoted")}{config?.drifted && <span className="error"> · {w("구성 변경 확인 필요", "Configuration drift detected")}</span>}</p></Section>
