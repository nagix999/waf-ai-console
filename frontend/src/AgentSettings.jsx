...
export default function AgentSettings({ onModels, onProductionChange }) {
  ...
  return <div className="page-stack agent-settings"><section className="panel agent-settings-panel"><h2>Agent 설정</h2><p className="ux-muted">Primary는 1차 판정, Verifier는 필요한 경우의 추가 검증을 담당합니다. 두 역할은 서로의 판정을 보지 않습니다.</p>
    ...
    <div hidden={section !== "models"} className="page-stack"><section className="panel agent-settings-panel"><div className="panel-head"><div><h2>역할별 모델</h2><p className="ux-muted">전체 검증을 통과한 프로필만 선택할 수 있습니다. 연결 정보와 검증은 LLM 프로필에서 관리합니다.</p></div><div className="ux-toolbar"><button className="secondary" disabled={state.busy} onClick={onModels}>LLM 프로필</button><button className="secondary" disabled={state.busy || state.loading} onClick={() => { setConfirm(false); void controller.current?.refresh(); }}>새로고침</button></div></div>
      ...
      })}<label className="checkbox-row"><input type="checkbox" checked={draft[purpose].evidence_editor_enabled === true} onChange={event => controller.current?.change(purpose, "evidence_editor_enabled", event.target.checked)} />근거 정리 사용</label>
      <label>근거 정리 모델<select disabled={!draft[purpose].evidence_editor_enabled} value={draft[purpose].evidence_editor_profile_id || ""} onChange={event => controller.current?.change(purpose, "evidence_editor_profile_id", event.target.value)}><option value="">Primary와 동일</option>{catalog.profiles.map(p => <option key={p.id} value={p.id} disabled={!p.can_assign}>{p.name}{!p.can_assign ? " · 검증 필요" : ""}</option>)}</select></label>
      <small className="ux-muted">반복된 근거를 묶고 기존 설명 중 대표 설명을 고릅니다. 판정은 바꾸지 않습니다. 정리할 후보가 있을 때 최대 1회 추가 호출하며, 실패하면 원래 근거를 보여줍니다.</small>
      </fieldset>)}</div><p className="ux-muted">테스트는 접수 시, 프로덕션은 최초 실행 시작 시 모델을 고정합니다. 진행 중 작업·기존 결과·실패 재실행의 모델은 바꾸지 않습니다. Primary와 Verifier 중 입력 여유가 작은 쪽에 맞춰 동일한 판정 입력을 전달합니다. 근거 정리 지침은 운영·테스트에 동일하게 적용합니다.</p>
