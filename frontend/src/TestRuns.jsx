...
export function TestRunItemRows({ items, onOpen, selection }) {
  return <div className="table-wrap"><table className={`test-run-items${selection ? " with-selection" : ""}`}><thead><tr>{selection && <th className="selection-cell">{selection.header}</th>}<th>문항 / 분류</th><th>판정 / 분석 요약</th><th>참고 답안 비교</th><th>처리 상태</th></tr></thead><tbody>{items.map(item => <tr key={item.id} className={analysisRowState(item.status).className}>{selection && <td className="selection-cell">{item.analysis_id && selection.cell(item.analysis_id, item.case_name || `${item.row_number}행`)}</td>}<td>{item.analysis_id ? <button type="button" className="text-button" onClick={() => onOpen(item.analysis_id)}>{item.case_name || item.event_id || `${item.row_number}행`}</button> : <strong>{item.case_name || item.event_id || `${item.row_number}행`}</strong>}<small>{item.row_number}행 · {item.difficulty || "난이도 미분류"} · {item.test_category || "유형 미분류"}</small></td><td><strong>{item.status === "completed" ? referenceVerdicts[item.verdict] || "판정 정보 없음" : "—"}</strong><SummaryPreview text={item.summary_ko} /></td><td>{item.ingest_status === "rejected" ? "접수 거부 · 평가 제외" : <CompactReferenceComparison evaluation={item.evaluation} />}</td><td>{item.ingest_status === "rejected" ? "접수 거부" : runStatuses[item.status] || "상태 미확인"}{item.error_code && <TestRunItemError item={item} />}</td></tr>)}</tbody></table></div>;
}
