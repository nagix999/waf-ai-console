...
export function TestRunRows({ items, onSelect, sorting, onSortingChange, expanded = false, production, words: w = ko => ko }) {
  ...
  const columns = [
    { id: "name", header: w("테스트", "Test"), sortable: true, render: run => <><button type="button" className="text-button" onClick={() => onSelect(run.id)}>{run.name}</button><small>{metadata(run)}</small><small>{context(run)}</small></> },
    ...(!expanded ? [{ id: "configuration", header: w("구성", "Configuration"), render: run => <>{run.execution_mode === "stub" ? w("모의 실행", "Stub") : identity(run)}<small>{run.prompt_version || "—"}</small></> }] : []),
  ...
  return <DataTable label={w("테스트 목록", "Tests")} data={items} columns={columns} headerGroups={expanded ? [{ label: "", span: 2 }, { label: w("정확도", "Quality"), span: 4 }, { label: w("판정 범위", "Decision coverage"), span: 2 }, { label: w("오류 방향", "Error direction"), span: 2 }, { label: "", span: 1 }] : undefined} sorting={sorting} onSortingChange={onSortingChange} rowClassName={run => analysisRowState(run.status).className} />;
}
