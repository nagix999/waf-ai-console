...
import { officialComparisonAllowed } from "./candidateConfiguration.js";
...
export function TestComparisonMetrics({ data }) {
  ...
  return <div className="comparison-quality"><p className="ux-muted comparison-metric-note">변화는 후보 − 기준입니다. 비율은 %p로 표시하며, 값이 커졌다고 항상 개선을 뜻하지는 않습니다.</p>
    <div className="comparison-metric-cards">{metrics.map(([key, label]) => <article key={key}><h3>{label}<MetricHelp metric={key} label={label} /></h3><div><span><small>기준</small>{metricText(columns[0]?.metrics?.[key], key)}</span><span aria-hidden="true">→</span><strong><small>후보</small>{metricText(columns[1]?.metrics?.[key], key)}</strong></div><p className="comparison-delta">{comparisonDelta(columns[0]?.metrics?.[key], columns[1]?.metrics?.[key])}</p></article>)}</div>
    <div className="table-wrap"><Table className="test-comparison-metrics"><thead><tr><th>동일 비교 문항 기준 지표</th><th>기준 실행</th><th>후보 실행</th><th>변화</th></tr></thead><tbody>
    <tr><th>비교 가능한 문항 수</th>{columns.map((summary, index) => <td key={index}>{countText(summary?.total)}건</td>)}<td>{comparisonDelta(columns[0]?.total, columns[1]?.total, false)}</td></tr>
    <tr><th>이진 확정 판정 수</th>{columns.map((summary, index) => <td key={index}>{countText(summary?.binary_decided)}건</td>)}<td>{comparisonDelta(columns[0]?.binary_decided, columns[1]?.binary_decided, false)}</td></tr>
    {counts.map(([key, label]) => <tr key={key}><th>{label}</th>{columns.map((summary, index) => <td key={index}>{countText(summary?.outcomes?.[key])}건</td>)}<td>{comparisonDelta(columns[0]?.outcomes?.[key], columns[1]?.outcomes?.[key], false)}</td></tr>)}
  </tbody></Table></div></div>;
}
...
export function TestComparisonItems({ items, onOpen }) {
  ...
  return <DataTable label="문항별 판정 변화" data={items} getRowId={item => item.pair_id || item.event_id} columns={[
...
export function TestComparisonResult({ data, state, onChange, onOpen, onRefresh, loading }) {
  ...
  return <div className="test-comparison-result">
    ...
    <DetailTabs label="비교 결과" items={[["quality", "판정 품질"], ["performance", "시간·사용량"], ["items", "문항별 변화"]]} value={tab} onChange={setTab}>{key => key === "quality" ? <TestComparisonMetrics data={data} /> : key === "performance" ? <TestComparisonPerformance performance={data.performance} /> : <>
...
export default function TestRunComparison({ candidateId, state: controlledState, onStateChange, onOpen, onUnauthorized, embedded = false }) {
  ...
  const showPicker = !state.baselineId || pickerOpen;
  useEffect(() => {
  ...
  const content = <section className="panel test-run-comparison">
    {state.open && <><div className="comparison-toolbar"><p className="ux-muted">같은 입력·답안의 저장된 결과만 비교하며 모델을 호출하지 않습니다. 모델·지침 변경만의 효과를 입증하는 실험은 아닙니다.</p><div className="ux-toolbar">{state.baselineId && <button type="button" className="secondary" aria-expanded={showPicker} onClick={() => setPickerOpen(value => !value)}>{showPicker ? "선택 닫기" : "비교 기준 변경"}</button>}<button type="button" className="secondary" disabled={catalog.loading || (state.baselineId && result.loading)} onClick={() => setReload(value => value + 1)}>새로고침</button></div></div>
      {showPicker && <div className="comparison-picker"><p>비교 기준이 될 테스트를 선택하세요.</p><form className="test-run-search" onSubmit={event => { event.preventDefault(); change({ type: "search" }); }}><label>비교 기준 테스트명 검색<input value={state.queryText} placeholder="테스트명으로 찾기" maxLength={120} onChange={event => change({ type: "draft", value: event.target.value })} /></label><button type="submit" className="secondary" disabled={catalog.loading}>검색</button></form>
      {catalog.loading && <p role="status">비교 기준 목록을 조회하는 중…</p>}{catalog.error && <p className="error" role="alert">{catalog.error}</p>}
      {catalog.data && <><div className="comparison-baselines" aria-label="비교 기준 선택">{catalog.data.items.filter(run => run.id !== candidateId).map(run => <button type="button" className="secondary" key={run.id} aria-pressed={state.baselineId === run.id} onClick={() => { change({ type: "baseline", id: run.id }); setPickerOpen(false); }}><strong>{run.name}</strong><small>{run.prompt_version || "프롬프트 미기록"} · {formatDate(run.created_at)}</small><span>{state.baselineId === run.id ? "선택된 비교 기준" : "이 실행을 기준으로 비교"}</span></button>)}</div>{!catalog.data.items.some(run => run.id !== candidateId) && <p className="muted">이 페이지에 비교할 다른 테스트가 없습니다. 다른 이름으로 검색하거나 페이지를 이동하세요.</p>}
  ...
  return embedded ? content : <><button type="button" className="secondary" onClick={() => change({ type: "open", value: true })}>다른 테스트와 비교</button><Dialog className="comparison-dialog" open={state.open} title="테스트 비교" onClose={() => change({ type: "open", value: false })}>{content}</Dialog></>;
}
