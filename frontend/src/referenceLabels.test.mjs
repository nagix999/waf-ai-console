...
const { CompactReferenceComparison, CompactEvaluationDetail, LabelAttachment, EvaluationSummary } = module.exports;
...
test("compact list comparison renders one criterion with a short status and explicit provenance", () => {
  ...
  assert.match(html, /합성 기대값 · AI 열람 미확인/);
  assert.doesNotMatch(html, /맞음|틀림|정확도|>AI 최종/);
});
...
test("exclusion reasons are collapsed, distinct, and not guessed from equal displayed verdicts", () => {
  const reasons = { failed: /실행 실패/, stub: /실제 LLM 판정이 아니므로/, unknown_provenance: /실제 모델 실행 여부/, input_contaminated: /Label 관련 키/ };
  for (const [outcome, reason] of Object.entries(reasons)) {
    ...
    assert.match(html, /<details class="compact-reference-reason"><summary>제외 사유/);
...
test("detail comparison does not repeat the AI verdict or read intermediate results", () => {
  ...
  assert.match(html, /<details class="compact-evaluation-provenance"><summary>출처·연결 이력/);
...
test("detail preserves all append-only history revisions and escapes supplied metadata", () => {
...
test("history loading, malformed data, empty history and refresh failure are distinct", () => {
  for (const [history, pattern] of [[null, /이력을 불러오는 중/], [undefined, /이력을 불러오는 중/], [{}, /이력 정보를 확인할 수 없습니다/], [[], /연결된 Label 이력이 없습니다/]]) {
    assert.match(render(CompactEvaluationDetail, { detail: fixture(), history }), pattern);
  }
  const html = render(CompactEvaluationDetail, { detail: fixture(), history: [], historyError: "<script>UNSAFE_ERROR_MESSAGE</script>" });
  assert.match(html, /마지막 조회 결과/);
  assert.doesNotMatch(html, /연결된 Label 이력이 없습니다|UNSAFE_ERROR_MESSAGE|script/);
});
...
test("controlled attachment can be opened by its parent without changing the dedicated form", () => {
  ...
  assert.match(closed, /<details class="panel label-attachment" hidden="">/);
  assert.match(opened, /<details class="panel label-attachment" open="">/);
  assert.match(standalone, /<details class="panel label-attachment">/);
  for (const html of [closed, opened, standalone]) {
    assert.match(html, /연결 미리보기/);
    assert.match(html, /명시적으로 선택하세요/);
    assert.match(html, /Source System \+ Event ID/);
    assert.match(html, /LLM 호출 없음/);
    assert.doesNotMatch(html, /Label 연결 확정/);
  }
});
