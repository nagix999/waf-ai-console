...
export default function TestReevaluation({ run, value, onChange }) {
  ...
  if (run.evaluation_mode === "ground_truth") return <div className="dataset-version-bar">
    <label>공식 평가 기록<select aria-label="공식 평가 기록" value={!value || ["latest", "initial"].includes(value) ? "initial" : value} onChange={event => onChange(event.target.value)}>
      <option value="initial">현재 결과 · 당시 답안 고정</option>{items.filter(item => item.evaluation_kind === "ground_truth").map(item => <option key={item.id} value={item.id}>평가 {item.revision} · {formatDate(item.created_at)}</option>)}
    </select></label><span className="ux-muted">{value && !["latest", "initial"].includes(value) ? "저장 당시 결과입니다. 이후 재실행 결과는 이 기록을 바꾸지 않습니다." : waiting ? "판정 진행 중 · 완료 후 평가 기록을 자동 저장합니다." : run.official_evaluation_pending ? "평가 기록을 저장하는 중입니다. 잠시 후 새로고침됩니다." : "평가 기록 저장됨 · 답안을 바꾸려면 Ground Truth에서 초안을 수정·발행한 뒤 새 테스트를 실행하세요."}</span>
