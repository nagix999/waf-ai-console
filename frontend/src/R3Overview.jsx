...
export function OfficialTrend({ points }) {
  const w = useWords();
  const chart = useRef(null); const [width, setWidth] = useState(680);
  useEffect(() => {
  ...
  const x = index => 45 + index * (width - 80) / Math.max(1, points.length - 1), y = value => 165 - value * 145;
  const labelEvery = Math.max(1, Math.ceil(points.length / Math.max(2, Math.floor(width / 90))));
  return <><svg ref={chart} className="r3-evaluation-chart" viewBox={`0 0 ${width} 200`} role="img" aria-label={w("같은 평가 기준의 운영 구성별 Accuracy, F1, Coverage 추이", "Accuracy, F1 and Coverage across Production configurations with matching evaluation scope")}>
    {[0, .25, .5, .75, 1].map(value => <g key={value}><line x1="45" x2={width - 35} y1={y(value)} y2={y(value)} /><text x="35" y={y(value) + 4} textAnchor="end">{value * 100}%</text></g>)}
    ...
    {points.map((point, index) => index % labelEvery === 0 || index === points.length - 1 ? <text key={point.id} x={x(index)} y="190" textAnchor="middle">{new Date(point.created_at).toLocaleDateString([], { month: "short", day: "numeric" })}</text> : null)}
...
export default function R3Overview({ onNavigate, onOpenRun, onPromotion, onGroundTruth, onFailures }) {
  ...
  const actionNames = { production_credentials: ["운영 API 키 발급", "Issue a Production API key"], production_traffic: ["운영 API 요청 확인", "Check Production API traffic"], ground_truth: ["답안·입력 확인", "Check Ground Truth cases"], failed_tests: ["실패한 테스트 실행", "Failed test executions"], failed_inference: ["실패한 운영 추론", "Failed Production inference"], promotion: ["운영 반영 검토", "Production Review"] };
  function openAction(action) { if (action.kind === "production_credentials") onNavigate("keys"); else if (action.kind === "production_traffic") onNavigate("api"); else if (action.kind === "ground_truth") onGroundTruth(action.dataset_id, "needs_attention"); else if (action.kind === "promotion") onPromotion(action.test_run_id); else onFailures(action.kind === "failed_tests" ? "test" : "production"); }
  return <div className="page-stack r3-overview">
    ...
    <div className="r3-overview-grid">{data?.evaluation && <Section title={points.length >= 2 ? w("운영 평가 추세", "Production Evaluation Trend") : w("현재 공식 평가", "Current Official Evaluation")} action={<NavigationAction onClick={() => onNavigate("quality")}>{w("평가 상세", "View evaluation")}</NavigationAction>}>
      ...
      {!draft ? <p className="v5-empty">{w("현재 공식 평가에 연결된 데이터셋이 없습니다. 다른 데이터셋을 임의로 표시하지 않습니다.", "No dataset is linked to the current official evaluation. No substitute dataset is selected.")}</p> : <><h3 className="r3-draft-title">{draft.name}</h3><div className="r3-stacked-bar" role="img" aria-label={w(`포함 가능 ${draft.counts.ready}, 확인 필요 ${draft.counts.needs_attention}, 제외 ${draft.counts.excluded}`, `Ready ${draft.counts.ready}, needs attention ${draft.counts.needs_attention}, excluded ${draft.counts.excluded}`)}>{["ready", "needs_attention", "excluded"].map(state => <span key={state} className={state} style={{ width: `${draft.total ? draft.counts[state] / draft.total * 100 : 0}%` }} />)}</div><dl className="r3-quality-counts">{[["ready", "포함 가능", "Ready to Include"], ["needs_attention", "확인 필요", "Needs attention"], ["excluded", "제외", "Excluded"]].map(([state, ko, en]) => <div key={state}><dt><i className={state} />{w(ko, en)}</dt><dd>{draft.counts[state]}</dd></div>)}</dl><p className="v5-context">{w("초안 변경", "Working changes")} {draft.working_changes_count} · {w("최근 공식 버전", "Latest published")} {draft.published_revisions[0] ? `r${draft.published_revisions[0].revision}` : "—"}</p></>}
    </Section>}</div>
    {comparable && <Section title={w("최근 동일 기준 테스트", "Recent Tests on the Same Baseline")} action={<NavigationAction onClick={() => onNavigate("runs")}>{w("전체 테스트", "All tests")}</NavigationAction>}><DataTable label={w("비교 가능한 테스트", "Comparable tests")} data={comparable ? data.recent_comparable_tests.filter(row => row.summary?.ground_truth?.comparison_key === data.comparison_key) : []} columns={[
      ...
      { id: "configuration", header: w("구성", "Configuration"), render: row => <>{row.summary?.profile_metadata?.model_name || row.profile_metadata?.model_name || "—"}<small>{row.summary?.prompt_version || row.prompt_version || "—"} · {row.summary?.input_schema_version || row.input_schema_version || "—"}</small></> },
      { id: "f1", header: "F1", className: "numeric", render: row => <span className="r3-mini-metric"><i style={{ width: `${(row.summary?.evaluation_summary?.metrics?.f1 || 0) * 100}%` }} />{percent(row.summary?.evaluation_summary?.metrics?.f1)}</span> },
      { id: "accuracy", header: "Accuracy", className: "numeric", render: row => percent(row.summary?.evaluation_summary?.metrics?.accuracy) },
      { id: "coverage", header: "Coverage", className: "numeric", render: row => percent(row.summary?.evaluation_summary?.metrics?.coverage) },
      { id: "time", header: w("평가 시각", "Evaluated"), render: row => formatDate(row.created_at) },
    ]} empty={w("같은 공식 버전·평가 범위의 테스트가 없습니다.", "No tests share this published revision and evaluation scope.")} /></Section>}
    <div className="r3-overview-grid"><Section title={w("할 일", "Action Center")}>{[["attention", w("확인 필요", "Needs Attention")], ["next", w("다음 단계", "Next Steps")]].map(([group, label]) => { const rows = (data?.actions || []).filter(action => (["promotion", "production_credentials", "production_traffic"].includes(action.kind) ? "next" : "attention") === group); return rows.length > 0 && <div key={group}><h3>{label}</h3><ul className="r3-action-list">{rows.map((action, index) => <li key={index}>{action.kind === "ground_truth" && action.datasets?.length ? <div><strong>{w("답안·입력 확인", "Check Ground Truth cases")} · {action.count}</strong>{action.datasets.map(dataset => <button key={dataset.dataset_id} onClick={() => onGroundTruth(dataset.dataset_id, "needs_attention")}><span>{dataset.name}</span><strong>{dataset.count}</strong><span>→</span></button>)}</div> : <button className={action.kind === "promotion" ? "positive" : ""} onClick={() => openAction(action)}><Icon name={action.kind === "promotion" ? "check" : group === "attention" ? "alert" : "arrow"} size={18} /><span>{w(...(actionNames[action.kind] || [action.kind, action.kind]))}<small>{action.name}</small></span><strong>{action.count ?? ""}</strong><span>→</span></button>}</li>)}</ul></div>; })}{data && !data.actions.length && <p className="v5-empty">{w("지금 확인할 항목이 없습니다.", "No actions need attention.")}</p>}</Section><Section title={w("최근 변경", "Recent Activity")} action={<NavigationAction onClick={() => onNavigate("changes")}>{w("전체 보기", "View all")}</NavigationAction>}><ul className="v5-activity-list">{activity.value?.items.map(row => <li key={row.id}><time>{formatDate(row.created_at)}</time><strong>{({ promotion: w("운영 반영", "Production application"), configuration: w("구성 변경", "Configuration"), runtime: "Runtime", integration: w("연동 변경", "Integration") })[row.category] || w("변경 기록", "Change")}</strong></li>)}</ul>{activity.error && <p className="error">{w("변경 이력을 조회하지 못했습니다.", "Could not load activity.")}</p>}{activity.value?.items.length === 0 && <p className="v5-empty">{w("기록된 변경이 없습니다.", "No changes recorded.")}</p>}</Section></div>
    <Section title={w("현재 운영 설정", "Current Production Configuration")} action={<NavigationAction onClick={() => onNavigate("agents")}>{w("구성 상세", "Configuration details")}</NavigationAction>}><ConfigurationRows snapshot={production?.snapshot} names={production?.profile_names} versions={production?.version_names} /></Section>
  </div>;
}
