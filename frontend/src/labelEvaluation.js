...
export const labelSources = { synthetic_expected: "합성 기대값", reference: "참조 Label" };
export const evaluationOutcomes = {
  match: "Label 일치", false_negative: "불일치 · 미탐 방향", false_positive: "불일치 · 과탐 방향",
  abstained: "모델 판단 보류", expected_abstention_match: "기대 보류 일치", expected_abstention_mismatch: "기대 보류 불일치",
  unlabeled: "미라벨", pending: "평가 대기", failed: "실행 실패 · 평가 제외", stub: "모의 실행 · 평가 제외",
...
export function evaluationOutcomeText(evaluation) {
  ...
  if (["false_negative", "false_positive"].includes(outcome)) {
    ...
    return `${reference?.source_kind === "synthetic_expected" ? "기대 불일치" : "틀림 · Label 기준"} · ${direction}`;
  }
...
export function evaluationExplanation(evaluation) {
  const outcome = evaluation?.outcome;
  if (outcome === "unlabeled") return "연결된 Label이 없어 평가하지 않습니다. 답안 연결은 분석 결과 목록에서 할 수 있습니다.";
  ...
  if (outcome === "stub") return "stub은 실제 LLM 판정이 아니므로 Label이 같아도 평가하지 않습니다.";
  if (outcome === "unknown_provenance") return "실제 모델 실행 여부를 확인할 수 없어 품질 평가에서 제외합니다.";
  if (outcome === "input_contaminated") return "기존 분석 입력의 추가 필드에서 Label 관련 키가 확인되어 평가에서 제외합니다. 이 검사는 임의의 원문에 숨긴 정답까지 탐지한다고 보장하지 않습니다.";
  if (outcome === "abstained") return "Label은 정탐/오탐이지만 모델 최종 판정은 보류입니다. 미탐·과탐과 구분하며 확정 판정 coverage에 반영합니다.";
  if (outcome?.startsWith("expected_abstention_")) return "합성 시나리오의 기대 보류 여부를 비교합니다. 정탐/오탐 확정 판정 지표와 분리하며 검증된 운영 정답이 아닙니다.";
  return "제공된 Label과 저장된 AI 최종 판정을 비교합니다. Label 자체의 정확성이나 실제 운영 성능을 보증하지 않습니다.";
}
...
// Keep the existing report/badge wording above unchanged for stored reports.
export function compactEvaluation(evaluation) {
  ...
  const source = reference?.source_kind === "synthetic_expected" ? "합성 기대값"
  ...
  return {
    ...
    explanation: unlabeled ? "연결된 참고 답안이 없습니다." : known ? evaluationExplanation(evaluation)
      : "평가 정보를 확인할 수 없습니다. 미라벨이나 평가 제외로 추정하지 않으며 새로고침으로 다시 확인하세요.",
  };
}
...
const attachmentErrors = {
  invalid_label_source_system: "Source System의 정확한 값과 길이를 확인하세요.",
  ...
  invalid_answer_event_id: "Event ID의 누락·앞뒤 공백·길이를 확인하세요.",
  invalid_reference_verdict: "기대 판정은 true_positive, false_positive, inconclusive 중 하나여야 합니다.",
  inconclusive_requires_synthetic_expected: "기대 보류(inconclusive)는 합성 기대값에서만 허용합니다.",
  duplicate_answer_event_id: "같은 Event ID가 답안 파일 안에 반복되었습니다.",
  analysis_not_found_in_source: "지정한 Source System에서 이 Event ID를 찾지 못했습니다. 분석 접수와 source 범위를 확인하세요.",
  label_preview_stale: "미리보기 이후 연결 대상이나 Label 이력이 변경되었습니다.",
...
export function labelAttachmentError(code) {
  return Object.hasOwn(attachmentErrors, code) ? `${attachmentErrors[code]} (${code})` : code || "Label 연결에 실패했습니다.";
}
...
export function validateLabelAttachment(file, form) {
  ...
  if (!form.source_system.trim()) return "연결할 Source System의 정확한 값을 입력하세요.";
  if (form.source_system.trim().length > 120) return "Source System은 120자 이내로 입력하세요.";
  if (!Object.hasOwn(labelSources, form.source_kind)) return "Label 출처를 선택하세요.";
