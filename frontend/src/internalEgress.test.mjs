...
test("settings render usage locks, private range guidance, missing data and errors without claiming connection validation", () => {
  ...
  assert.match(html, /<h2>내부 연결 허용<\/h2>/); assert.match(html, /사용 중인 대상은 설명만 수정할 수 있습니다/); assert.match(html, /주소 변경·삭제는 연결 프로필을 모두 비활성화한 뒤/); assert.doesNotMatch(html, /내부 연결 허용 설명|사용 중인 내부 대상 설명|revision|<form/);
  assert.match(html, /aria-label="10.0.0.10:8000 관리" aria-expanded="false"/);
  const source = readFileSync(new URL("./InternalEgressSettings.jsx", import.meta.url), "utf8");
  assert.match(source, /RFC1918 IPv4와 ULA IPv6/); assert.match(html, /OpenAI 설정에는 영향이 없으며 방화벽 설정·연결 검증을 대신하지/); assert.match(source, /readOnly=\{inUse\}/); assert.match(source, /disabled=\{blocked \|\| item.in_use_profiles.length > 0\}/); assert.match(source, /닫기 · 입력 유지/);
