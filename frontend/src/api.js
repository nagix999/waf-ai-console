...
export const api = {
  ...
  overview: (options = {}) => request("/api/v1/admin/production-configurations/overview", { ...options, cache: "no-store" }),
  workingDataset: (id, query = {}, options = {}) => request(`/api/v1/validation-datasets/${encodeURIComponent(id)}/working/search`, { ...options, method: "POST", body: JSON.stringify(query), cache: "no-store" }),
  workingItem: (id, item, options = {}) => request(`/api/v1/validation-datasets/${encodeURIComponent(id)}/working/items/${encodeURIComponent(item)}`, { ...options, cache: "no-store" }),
  saveWorkingItem: (id, item, payload) => request(`/api/v1/validation-datasets/${encodeURIComponent(id)}/working/items${item ? `/${encodeURIComponent(item)}` : ""}`, { method: item ? "PUT" : "POST", body: JSON.stringify(payload) }),
  workingBulk: (id, payload) => request(`/api/v1/validation-datasets/${encodeURIComponent(id)}/working/bulk`, { method: "POST", body: JSON.stringify(payload) }),
  importWorkingAnalyses: (id, payload) => request(`/api/v1/validation-datasets/${encodeURIComponent(id)}/working/imports`, { method: "POST", body: JSON.stringify(payload) }),
  restoreWorking: (id, item, payload) => request(`/api/v1/validation-datasets/${encodeURIComponent(id)}/working/${item ? `items/${encodeURIComponent(item)}/revert` : "discard"}`, { method: "POST", body: JSON.stringify(payload) }),
  publishDataset: (id, payload) => request(`/api/v1/validation-datasets/${encodeURIComponent(id)}/publish`, { method: "POST", body: JSON.stringify(payload) }),
  updateWorkingMetadata: (id, payload) => request(`/api/v1/validation-datasets/${encodeURIComponent(id)}/working/metadata`, { method: "PATCH", body: JSON.stringify(payload) }),
