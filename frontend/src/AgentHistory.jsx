  ...
  return <section className="agent-history inspection-history">
    ...
    <div className="inspection-agent-grid">
      ...
      {step ? <section className="inspection-step-detail"><div className="panel-head-inline"><h3>{stepLabel(step.step_type || step.name, step.metadata)}</h3></div><p className="step-timing"><span className={`status status-${step.status}`}>{status(step.status)}</span><strong>{executionDuration(step)}</strong><small>재시도 포함</small><small>{formatDate(step.started_at)} → {step.completed_at ? formatDate(step.completed_at) : "종료 기록 없음"}</small></p>
        {step.metadata?.model_profile && <p className="ux-muted">{step.metadata.model_profile} · {step.metadata.model_name}</p>}
        {step.metadata?.evidence_grounding_retry?.attempted && <p className="notice">근거 오류 재시도 1회 · {step.metadata.evidence_grounding_retry.recovered ? "원문 인용 검사 통과" : "교정 미완료"}. 교정 전 출력은 처리 결과의 grounding_history에서 확인할 수 있습니다.</p>}
        ...
        <TextInspector jsonText key={`${run.id}-${step.id}-${tab}`} label={tab === "output" ? "단계 처리 결과" : tab === "input" ? "단계 분석 입력" : "단계 실행 정보"} value={tab === "metadata" ? { metadata: step.metadata, tool_calls: step.tool_calls } : step[tab]} />
