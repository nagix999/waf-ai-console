...
> 2026-09-16 출력 복구: [응답 교정 재시도 계약](docs/LLM_Output_Recovery_2026-09-16.md)이 아래 출력 형식 교정 1회 설명에 우선한다. Primary/Verifier는 형식 오류·명시적 생성 중단을 원인별로 최초 포함 최대 4회 처리한다. 판정·근거 검증·독립성·모델/출력 한도·기존 결과는 유지하며 DB/패키지 변경은 없다. 운영 성공률·실제 모델 품질은 내부 재검증이 필요하다.

> 2026-09-14 후속 2·3차 보완: [부분 로그·보류 안내 보완](docs/Partial_Input_and_Hold_Review_2026-09-14.md)은 v2.12로 고정된 새 실행에 request-integrity-v2를 적용하고, 보류 상세·보고서에서 저장된 확인 조건 또는 근거 검토를 안내한다. 지침 문구·독립 Verifier·DB·모델 설정은 유지한다. 과거 v2.9~v2.11은 v1 검사로 재실행하고 기존 결과를 덮어쓰지 않는다. 재배포·실제 모델 검증은 별도다.

> 2026-09-14 후속 1차 보완: [요청 의미 판단 v2](docs/Context_Judgment_v2_2026-09-14.md)는 공통 지침 v2.11의 판단 순서를 명확히 하고 별도 검수 자료 18건을 추가한다. 기존 Primary → 조건부 독립 Verifier → 서버 정책, 과거 v2.6~v2.10 스냅샷과 근거 계약을 유지한다. DB·모델·입력 구조·운영 설정 변경은 없으며 코드·오프라인 검증 범위다. 실행 중 서비스는 v2.10을 유지하고 실제 모델 재평가는 아직 하지 않았다.

> 2026-09-14 후속 평가: 배포된 v2.10·gpt-5.4-mini로 파싱/Hard/Medium 각 50건의 실제 평가를 완료했다. v2.8 대비 답안 일치는 119→120건이나 미탐 0→1건이 남아 운영 확대를 권고하지 않는다. [비교·설명 품질·한계](docs/evaluations/Analyst_Assessment_GPT54Mini_150_2026-09-14.md)를 참고한다. 아래 미검증 안내는 각 구현 당시 기록이며 이번에 설계·설정·기존 결과를 변경하지 않았다.

> 2026-09-14 분석가 설명: [근거 구분·보류 설명 1~3단계](docs/Analyst_Evidence_and_Hold_Explanation_2026-09-14.md)는 v2.10의 새 실행에서 근거 방향·핵심 구분점과 두 역할의 원문 대조된 근거를 기록한다. 기존 최대 5개 최종 정책 근거와 최대 10개 화면용 근거를 분리해 판정 규칙은 유지한다. 과거 결과·고정 지침을 재작성하지 않으며 DB·운영 패키지·추가 LLM 단계는 없다. 코드·오프라인 검증 범위이고 새 배포·실제 모델 평가는 별도다.

> 2026-09-11 입력 검사: [요청 입력 검사 v1](docs/Request_Integrity_v1_2026-09-11.md)이 새 공통 지침 v2.9의 완전성·최종 제한 계약에 우선한다. 기존 파싱 상태와 별도로 누락·경계·JSON 구조를 관찰하며, 해당 구간에 근거한 정상 판정에만 제한적으로 보류·확인 안내를 적용한다. 별도 LLM 파싱·외부 조회·운영 의존성·DB 변경 없이 과거 지침과 결과는 보존한다. 코드·오프라인 검증 범위이며 새 배포·실제 모델 재평가는 하지 않았다.

> 2026-09-10 새 실행의 원문 계약과 지침은 [원문 후보 선택 v1](docs/Evidence_Selection_v1_2026-09-10.md) 및 [요청 문맥 판단 v1](docs/Context_Judgment_v1_2026-09-10.md)이 우선한다. 공통 지침 v2.8에서 데이터·설정·인용문과 공격 실행 입력을 구분하도록 보강하되 출처 선택·Verifier·이전 스냅샷은 유지한다. 별도 LLM 파서·조사 도구·DB/의존성 추가는 없다.

> 2026-09-10 인용 교정 v2: [원문 인용 교정 계약](docs/Evidence_Citation_Repair_v2_2026-09-10.md)이 근거 오류의 전체 재작성 설명에 우선한다. 디코딩 original의 실제 제출 위치 연결, 제출된 필드·구간 검증, 인용만 교정하는 제한된 출력 계약, 유효 근거 보존과 미해결 시 보류, 고정 출력 오류 코드를 구현한다. 기본 판정 지침·모델·Verifier 결합·과거 데이터는 유지하며 DB/의존성 변경 없이 코드·오프라인 검증만 수행한다.

> 2026-09-10 역할별 LLM·근거 검증: [Agent 설정 및 근거 검증 계약](docs/Agent_Settings_and_Grounding_v0.2.0.md)이 아래 단일 모델·근거 탈락 즉시 보류 설명에 우선한다. `0014_agent_configuration`으로 Production/Test의 Verifier를 별도 지정하고 기존 Primary·공통 지침은 유지한다. 파서와 원문 위치 검증을 통일하고 근거 오류에 역할별 1회 교정을 추가한다. Test는 접수 시, Production은 최초 실행 시 두 모델을 고정하며 v2 실행 스냅샷은 재선점/실패 재실행에 재사용한다. 보류 사유·교정 전 구조화 출력의 암호화 이력을 기록하고 Verifier 독립성·실패/불일치 보류 정책은 유지한다. LLM 보조 파싱과 조사 도구는 이번 범위가 아니다.
