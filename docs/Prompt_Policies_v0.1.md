...
2026-09-07 기본 범위: 저장·복제·변경 비교·공통 적용·이전 저장 버전으로 복귀. 2026-09-08에는 Production과 Test가 같은 프롬프트를 쓴다는 계약을 유지하면서 공통 지침 v2.6 경량화와 실행 간 비교를 추가했다. [공통 프롬프트·실행 비교 안내](Prompt_Optimization_and_Comparison_v0.2.0.md)를 따른다. 테스트별 후보 선택·자동 품질 승인·운영 승격은 하지 않는다.
...
새 이벤트를 접수할 때 정책 ID·내용 지문과 함께 두 역할의 **전체 지침**을 암호화한 스냅샷을 저장한다. 현재 코드 지침은 `waf-judgment-v2.6`, 고정 규칙은 `waf-system-v2.6`이며 상세의 `prompt_version`은 `waf-judgment-v2.6/policy-N` 형식이다. 정책 ID는 추가 응답 필드 `prompt_policy_version_id`로 확인한다.
...
접수 응답의 버전은 접수 당시 공통 활성 지침이며 실행 완료를 뜻하지 않는다. stub 실행은 실제 사용 버전을 `stub-v0`으로 표시한다. stub에 연결된 정책 ID를 실모델 품질 결과로 해석하면 안 된다. Production은 기존 모델 선택 정책을 유지한다. 이름 있는 일반 테스트는 접수 당시 Test 지정 모델과 공통 활성 프롬프트를 고정한다. 테스트 접수는 Production 모델이나 공통 활성 지침을 변경하지 않는다.
