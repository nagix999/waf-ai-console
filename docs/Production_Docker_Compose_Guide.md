...
기준: 2026-09-10 Agent 설정·근거 검증 개선 소스, 애플리케이션 v0.2.0, DB `0014_agent_configuration`. 이 문서 갱신은 운영 배포를 수행했다는 뜻이 아니다.
...
**기존 `team-workspace-platform`의 원본 파일을 수정하지 않고 연결하려면 [환경 파일 기반 자동 배포](Automated_Team_Deployment.md)를 먼저 사용한다.** 이 문서는 호스트 프록시 또는 개별 단계를 직접 관리할 때의 대안이다. 자동 배포가 생성한 키·실행 설정은 입력 `.env.production`에 다시 쓰지 않으므로, 자동 배포와 이 문서의 수동 Compose 명령을 섞어 실행하지 않는다.
...
위 그림은 호스트 HTTPS 프록시 방식이다. 기존 **Docker Gateway**에 `waf.cyberailabs.team`을 추가하는 경우에는 [자동 배포 안내](Automated_Team_Deployment.md)를 권장한다. 기존 프로젝트의 설정을 직접 수정하는 대안은 [서브도메인 수동 연결 안내](Subdomain_Deployment_CyberAILabs.md)에 보존한다. 두 방식 모두 전용 공유 네트워크로 Gateway → `waf-web:80`을 연결하며, Gateway 컨테이너에서 `127.0.0.1:18080`을 사용하지 않는다. 회사/VPN 접근망은 기존 방화벽에서 관리하고 WAF에 중복 등록하지 않는다.
...
현재 공개 저장소다. 조직 정책이나 저장소 접근 설정 때문에 인증이 필요하면 승인된 GitHub 인증을 사용하고 토큰을 clone URL에 넣지 않는다. 검토한 커밋으로 배포하고 커밋 SHA를 배포 기록에 남긴다. **최초 `v0.2.0` 태그가 아니라 이 가이드가 포함된 최신 검증 커밋**을 사용한다. 태그를 이동하거나 기존 이력을 덮어쓰지 않는다.
...
예제의 `WAF_TEAM_PROJECT_DIR`, `WAF_TEAM_PROJECT_NAME`, `WAF_EDGE_SUBNET`, `WAF_EDGE_IP_RANGE`는 자동 배포 전용이다. 이 수동 구성에서는 사용하지 않는다. 자동 배포용 빈값 보완도 수행하지 않으므로 위 표의 비밀값과 네트워크 필수값을 직접 채워야 한다.
...
Alembic 현재 버전은 `0014_agent_configuration`, readiness 응답은 `{"status":"ok"}`여야 한다. HTTPS 확인 주소는 검토한 public origin으로 맞춘다. 브라우저용 웹의 loopback 주소를 curl로 직접 호출하면 승인 프록시 검사를 통과하지 못할 수 있으며 이를 피하려고 검사를 끄지 않는다. API 기본 기동 명령도 `upgrade head`를 수행하지만 worker 시작 전에 명시적으로 버전을 확인한다. readiness는 DB 접속 수준이며 GPU·worker 처리·판정 품질을 보장하지 않는다.

HTTPS 브라우저에서 로그인, Secure 쿠키, 설정 → Agent 설정 → 공통 지침과 입력 스키마 조회, `Production API` 화면과 PDF 다운로드를 확인한다. 신규 DB에서는 기본 스키마·지침 조회가 초기화·감사 기록을 만들 수 있다. 아직 테스트/분석 버튼은 누르지 않는다.
...
1. **설정 → Agent 설정 → 공통 지침**, **설정 → 입력 스키마**: 기본 활성 버전을 확인한다. 변경할 필요가 없다면 다시 저장·적용하지 않는다. Test와 Production은 공통 활성 프롬프트를 사용한다.
...
5. **설정 → Agent 설정 → 모델 배정**에서 테스트 Primary를 지정하고 Verifier는 **Primary와 동일** 또는 별도 검증된 프로필을 선택한다. 배정을 저장한 후 승인된 가상 입력 소수로 **테스트 분석 → 단건 분석**을 확인한다.
6. 같은 화면에서 프로덕션의 Primary·Verifier도 지정한다. 같은 검증 프로필을 여러 역할에 지정해도 되며 같은 검증을 역할별로 반복할 필요는 없다. 서로 다른 프로필이면 각각 검증한다. 미지정 Test를 Production으로 대체하지 않는다. 후보 150건 검증은 선택한 후보 모델만 양 역할에 사용하므로 두 모델 조합의 품질은 일반 테스트로 별도 확인한다.
