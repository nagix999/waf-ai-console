...
GitHub `main` 반영도 함께 하려면 [통합 배포 스크립트](Unified_Deployment_Script.md)의 `bash scripts/deploy.sh`를 사용하세요. 이 문서의 `redeploy.sh`는 Git 작업 없이 현재 작업 폴더로 배포하는 용도로 유지합니다.
...
이 스크립트로 한 번 재배포한 컨테이너에는 env 경로가 `/dev/null`로 기록될 수 있습니다. 값이 고정된 Compose에서 추가 env를 읽지 않는 정상 설정이며, 이를 누락 파일로 거부하던 오류를 보완했습니다. 기록된 상대경로는 원래 배포 폴더 기준으로 해석합니다. 실제 누락된 env는 계속 거부하며 오류에 확인할 경로를 표시합니다.
