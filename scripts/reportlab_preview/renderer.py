"""Isolated ReportLab design prototype. Not imported by the service or routes.

Accepts the normalized, allowlisted web report AST produced by prepare.mjs.
No browser, network fetches, database access, or dependency installation.
"""
...
FONT_DIR = ROOT / "backend/app/assets/fonts"
...
class LimitedBuffer(BytesIO):
    def write(self, data):
        if self.tell() + len(data) > MAX_PDF_BYTES:
            raise PreviewError("preview_pdf_too_large")
        return super().write(data)


def register_fonts():
    for name, file in (("PreviewRegular", "NanumGothic.ttf"), ("PreviewBold", "NanumGothicBold.ttf")):
...
def literal(value: str, *, preserve_spaces=False) -> str:
    ...
    glyphs = pdfmetrics.getFont("PreviewRegular").face.charToGlyph
    text = "".join(
...
def styles():
    base = ParagraphStyle("body", fontName="PreviewRegular", fontSize=9, leading=14,
                          textColor=INK, wordWrap="CJK", splitLongWords=True,
                          spaceAfter=7, allowWidows=0, allowOrphans=0, alignment=TA_LEFT)
    return {
        "body": base,
        "intro": ParagraphStyle("intro", parent=base, fontSize=8, leading=12, textColor=MUTED, spaceAfter=3),
        "title": ParagraphStyle("title", parent=base, fontName="PreviewBold", fontSize=25, leading=33, spaceAfter=10, keepWithNext=True),
        "section": ParagraphStyle("section", parent=base, fontName="PreviewBold", fontSize=13, leading=19, spaceBefore=17, spaceAfter=9, keepWithNext=True),
        "sub": ParagraphStyle("sub", parent=base, fontName="PreviewBold", fontSize=10, leading=15, spaceBefore=10, spaceAfter=6, keepWithNext=True),
        "label": ParagraphStyle("label", parent=base, fontSize=8, leading=12, textColor=MUTED, spaceAfter=5, keepWithNext=True),
        "cell": ParagraphStyle("cell", parent=base, fontSize=8.5, leading=13, spaceAfter=0),
        "key": ParagraphStyle("key", parent=base, fontName="PreviewBold", fontSize=8, leading=13, textColor=MUTED, spaceAfter=0),
        "code": ParagraphStyle("code", parent=base, fontSize=8, leading=12, backColor=PAPER,
...
def evidence_card(title, rows, sheet):
    ...
    border = colors.Color(*[(component + 3) / 4 for component in
...
def badge_style(value, parent):
    ...
    if not background:
        return parent, None
    return ParagraphStyle("badge", parent=parent, fontName="PreviewBold", textColor=colors.white), colors.HexColor(background)
...
def report_table(block, sheet, *, compact=False, width=WIDTH):
    ...
    if not key_value:
        ...
        formatting.append(("BACKGROUND", (0, 0), (-1, 0), PAPER))
    for row in rows:
        cells = []
        row_index = len(data)
        for index, text in enumerate(row):
    ...
    if not data:
        return Spacer(1, 1)
    if key_value:
        formatting.append(("BACKGROUND", (0, 0), (0, -1), PAPER))
...
def validate_document(document):
    if not isinstance(document, dict) or not isinstance(document.get("title"), str):
        raise PreviewError("preview_invalid_document")
    ...
    for block in blocks:
        if not isinstance(block, dict):
            raise PreviewError("preview_invalid_block")
        kind = block.get("type")
        if kind in {"paragraph", "heading", "code"}:
            ...
            if (not isinstance(headers, list) or not 1 <= len(headers) <= 8
                ...
                raise PreviewError("preview_invalid_table")
            count += len(rows)
        else:
            raise PreviewError("preview_unknown_block")
    if count > MAX_BLOCKS:
        raise PreviewError("preview_too_many_blocks")
...
def render_pdf(document):
    ...
    card_sheet["code"] = ParagraphStyle("card-code", parent=sheet["code"], backColor=PAPER)
    ...
    for number, section in enumerate(document["sections"], 1):
        story.append(paragraph(f"{number:02d}   {section['title']}", sheet["section"]))
    ...
    def furniture(canvas, doc):
        if doc.page > MAX_PAGES:
            raise PreviewError("preview_too_many_pages")
        canvas.saveState()
        canvas.setStrokeColor(ACCENT)
        canvas.setLineWidth(2)
        canvas.line(44, A4[1] - 30, A4[0] - 44, A4[1] - 30)
        canvas.setFont("PreviewBold", 7)
        canvas.setFillColor(ACCENT)
        canvas.drawString(44, A4[1] - 45, "WAF AI  /  ANALYSIS REPORT")
        canvas.setFillColor(MUTED)
        canvas.setFont("PreviewRegular", 7)
        canvas.drawRightString(A4[0] - 44, A4[1] - 45, "디자인 시안 · 가상 데이터 · LLM 미실행")
        canvas.setStrokeColor(LINE)
        canvas.setLineWidth(0.5)
        canvas.line(44, 40, A4[0] - 44, 40)
        canvas.drawString(44, 26, "ReportLab 시안 02 · 운영 보고서가 아닙니다")
    ...
    doc = SimpleDocTemplate(buffer, pagesize=A4, leftMargin=44, rightMargin=44,
                           ...
                           title="WAF 분석 보고서 · 디자인 시안", author="WAF AI Console",
                           allowSplitting=1)
    ...
    try:
        ...
        for code in ("preview_too_many_pages", "preview_pdf_too_large"):
            if isinstance(error, PreviewError) and str(error).endswith(code):
                raise PreviewError(code) from None
        raise PreviewError("preview_render_failed") from None
    return buffer.getvalue()


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("directory", type=Path, help="Fresh output folder prepared by prepare.mjs")
    args = parser.parse_args()
    for name in ("01-short", "02-long", "03-evidence"):
        source = args.directory / f"{name}.json"
        if source.stat().st_size > MAX_INPUT_BYTES:
            raise PreviewError("preview_input_too_large")
        content = render_pdf(json.loads(source.read_text(encoding="utf-8")))
        with (args.directory / f"{name}.pdf").open("xb") as handle:
            handle.write(content)
        print(f"{name}.pdf: {len(content)} bytes")
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit("preview_render_failed") from None
