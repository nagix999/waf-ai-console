...
from ..models import AccessAudit, Analysis, AnalysisLabel, TestRunItem, ValidationDatasetItem, ValidationDatasetWorkingItem
...
def purge(db, key_id, request, actor):
    ...
    for item_id in db.scalars(select(ValidationDatasetWorkingItem.id).where(ValidationDatasetWorkingItem.original_analysis_id.in_(scope))):
        db.add(AccessAudit(actor_kind="admin_session", actor_id=actor,
            action="detach_deleted_analysis_source", resource_type="validation_dataset_working_item", resource_id=item_id))
    db.execute(update(ValidationDatasetWorkingItem).where(ValidationDatasetWorkingItem.original_analysis_id.in_(scope))
        .values(original_analysis_id=None, original_analysis_deleted=True))
