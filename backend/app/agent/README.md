...
2026-09-21 **Canonical V5 운영 승격**: Primary → 조건부 독립 Verifier → 서버 정책 → 선택적 근거 정리 순서를 유지합니다. 새 UI 테스트는 모델·저장 지침·입력 스키마를 명시적으로 선택하고 실행 구성을 고정합니다. Production의 다섯 구성 요소는 승인된 Ground Truth 공식 평가를 마친 테스트를 관리자 승격 API로 적용할 때만 함께 변경됩니다. Test 기본 모델과 동시 처리 설정은 승격 대상이 아닙니다. 이미 접수한 분석·재실행은 기존 고정 설정을 사용하고 필드 설명은 계속 `history_only`입니다. [V5 구현·호환성·검증](../../../docs/v0.3.0/CANONICAL_V5_IMPLEMENTATION.md)이 아래 과거 문서의 직접 활성화·미구현 상태 설명보다 우선합니다. 실제 모델 호출·운영 DB 이전·재배포는 이번 작업에서 수행하지 않았습니다.

2026-09-17 **공식 평가 연결**: 데이터셋의 승인된 문항·답안과 후보 구성을 접수 시 고정하고, 판정 완료 후 worker가 별도 평가 기록을 저장합니다. 실패 재실행은 새 평가 기록을 만들며 이전 기록은 보존합니다. 평가 저장 실패는 완료된 분석을 실패로 바꾸지 않습니다. 승인 상태·답안·필드 설명을 모델에 추가로 전달하거나 새로운 Agent 단계를 만들지 않습니다. [공식 평가 계약](../../../docs/v0.3.0/OFFICIAL_EVALUATIONS.md). 운영 승격·실제 LLM 검증·배포는 아직 수행하지 않았습니다.

2026-09-17 **v0.3.0 개발 중 · 테스트 후보 구성**: 단건/파일/데이터셋 테스트 요청에 `candidate_configuration`을 지정하면 검증된 Primary·Verifier·근거 정리 모델과 저장된 지침·스키마 버전을 실행별로 고정합니다. 비활성 지침·스키마를 선택해도 운영 활성 설정은 바꾸지 않습니다. 구성 정보는 이력이며 모델 입력이 아니고, 필드 설명도 계속 `history_only`입니다. 기본 설정으로 요청하면 기존 Test 역할과 공통 활성 버전을 사용합니다. Primary→조건부 독립 Verifier→서버 판정 정책, 교정 재시도, 과거 실행의 고정 지침은 유지합니다. [현재 구현 계약과 미완료 범위](../../../docs/v0.3.0/GROUND_TRUTH_AND_CANDIDATES.md)를 참고하세요.
...
2026-09-15 검증 데이터 관리: 데이터셋 문항은 별도 암호화된 버전으로 복사하며 TestRun의 입력·답안·모델을 고정한다. 개별/일괄 참고 답안과 메모는 LLM 입력에 포함하지 않는다. 재평가는 답안 비교만 수행하고 모델을 호출하지 않는다. 운영에서 가져온 문항은 수정 후에도 `internal_only`를 유지한다. 접수 시 Primary·Verifier·근거 정리 프로필을 확인하고 각 요청 직전에도 vLLM 여부를 검사한다. 재실행에는 이 제한을 복사한다. 외부 모델로 자동 대체하지 않는다. Test 서비스 키는 Test 역할·동시 처리 설정만 사용하며 Production 결과 접근을 차단한다. 실제 사내 로그를 개발 fixture에 넣지 않는 원칙은 그대로다.

관리자가 별도 파일로 연결한 정답 Label은 추가형 평가 이력에만 저장하고 이벤트·fingerprint·Primary/Verifier 입력에 넣지 않습니다. Test·Production 단건 요청과 파일 업로드는 최상위 `expected_verdict`를 입력 경계에서 먼저 분리한 뒤 같은 트랜잭션으로 연결합니다. 나머지 예약 평가 필드는 계속 거부합니다. payload 본문·임의 중첩 필드 속 정답을 제거하는 기능은 아닙니다. 비교 대상은 정책 결합과 원문 근거 검증을 마친 최종 판정이며, Primary 단독 출력이나 WAF Action을 정답으로 사용하지 않습니다. 비교 집계 자체는 LLM을 다시 호출하지 않습니다. 합성 기대값, AI 열람 여부 미확인 답안, stub·실행 출처 불명 결과를 검증된 독립 품질로 표시하지 않습니다. 이미 모델에 전달된 정답 정보는 사후 Label 연결로 정화되지 않습니다.
...
# 선택적 근거 정리 — 2026-09-14

2026-09-15 확장: 새로 고정되는 선택적 정리 지침 `result-editor-v2`는 근거와 최종 추가 확인사항(기존 최대 5개)을 한 번에 정리한다. 동일 자료 안에서 같은 작업·조건인 경우 기존 대표 항목의 번호를 선택하고, 모든 번호의 중복 없는 포함을 검사한다. 확인사항 표시 매핑은 `follow-up-editor-v1`로 원래 목록에 연결하며 원본·판정은 유지한다. 자료명이 다르면 병합을 거부하지만 같은 자료 안의 의미 동등성은 모델 판단이므로 실제 품질 검증은 별도다. 이전 `evidence-editor-v1` 스냅샷은 이전 계약을 유지한다. 모델 배정·호출 한도·동시 처리·DB는 변경하지 않는다. [확장 범위·보존·검증](../../../docs/Follow_Up_Organization_2026-09-15.md)을 따른다. 아래 내용은 이전 버전의 기록이다.

서버 최종 판정 뒤 `llm_evidence_editor` 단계를 선택적으로 실행한다. 같은 출처·근거 방향의 반복 설명을 묶고 기존 설명 중 대표를 번호로 선택한다. 정탐·오탐 교차 병합·누락을 거부하고, 실패하면 원래 근거를 표시하며 최종 판정은 바꾸지 않는다. 기본 꺼짐이며 Agent 설정에서 운영/테스트별 검증 프로필을 배정한다. 두 용도의 지침은 동일하고 접수/실행 당시 설정을 암호화해 고정한다. 실행 스냅샷 v1/v2·과거 테스트·후보 150건 검증에는 소급 적용하지 않는다. 공통 동시 처리 슬롯을 사용하며 최대 한 번 호출한다. 저장된 표시 매핑은 화면/Markdown/PDF/Excel에 동일하게 적용한다. 신규 의존성은 없고 Alembic `0016_evidence_editor`가 필요하다. [세부 계약·한도·평가 기준](../../../docs/Evidence_Editor_and_Three_Way_Evaluation_2026-09-14.md).
