...
def get_principal(request: Request, supplied: Annotated[str | None, Depends(service_api_key)]) -> Principal:
    ...
    try:
        ...
        with request.app.state.session_factory() as db:
            ...
            if key is not None:
                principal = Principal(kind="service_api_key", source_system=key.source_system, scopes=frozenset(key.scopes_json), service_api_key_id=key.id, purpose=key.purpose)
                db.commit()
                return principal
