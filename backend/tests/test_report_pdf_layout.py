"""Offline layout checks using fictional documents only."""
import copy
import json
...
from app.services import report_pdf_layout as renderer
...
def test_invalid_document_has_safe_error(value):
    with pytest.raises(renderer.ReportLayoutError, match="^report_invalid_document$"):
...
def test_rejects_unknown_or_malformed_blocks(block):
    with pytest.raises(renderer.ReportLayoutError, match="^report_"):
        renderer.render_pdf(document(block))


def test_size_limits(monkeypatch):
    with pytest.raises(renderer.ReportLayoutError, match="^report_input_too_large$"):
        renderer.render_pdf(document({"type": "code", "text": "x" * renderer.MAX_INPUT_BYTES}))
    monkeypatch.setattr(renderer, "MAX_BLOCKS", 2)
    with pytest.raises(renderer.ReportLayoutError, match="^report_too_many_blocks$"):
...
def test_page_limit(monkeypatch):
    ...
    with pytest.raises(renderer.ReportLayoutError, match="^report_too_many_pages$"):
...
def test_output_limit(monkeypatch):
    ...
    with pytest.raises(renderer.ReportLayoutError, match="^report_pdf_too_large$"):
...
def test_dark_palette_keeps_explanations_unfilled_and_has_readable_heading():
    renderer.register_fonts()
    sheet = renderer.styles("dark")
    card = renderer.evidence_card("정탐 근거", [[renderer.paragraph("설명", sheet["body"])]], sheet)
    assert card._bkgrndcmds[0][-1] == sheet["palette"]["page"]
    assert card._bkgrndcmds[1][-1] != sheet["palette"]["page"]
    assert sheet["body"].backColor is None
    assert sheet["code"].backColor == sheet["palette"]["paper"]
    assert renderer.render_pdf(document(), theme="dark").startswith(b"%PDF-")


@pytest.mark.parametrize("block", [
    {"type": "heading", "level": "private", "text": "text"},
    {"type": "heading", "level": True, "text": "text"},
    {"type": "list", "ordered": "private", "items": ["a"]},
    {"type": "list", "start": -1, "items": ["a"]},
])
def test_malformed_structure_fails_closed(block):
    with pytest.raises(renderer.ReportLayoutError, match="^report_invalid_block$"):
        renderer.render_pdf(document(block))


def test_pdf_has_section_bookmarks_without_external_actions():
    content = renderer.render_pdf(document())
    assert b"/Outlines" in content
    assert b"/URI" not in content and b"/Launch" not in content
